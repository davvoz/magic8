...
**Vocabulary (fixed in code, combined in data):** `draw_card`, `deal_damage`, `heal`, `modify_stats`, and since the Shadow content drop `drain` (damage that heals the controller), `discard` (a player discards at random, seeded) and `sacrifice` (a creature goes to the graveyard through the ordinary death pipeline).

| Concept | Data expresses | Validated by |
|---|---|---|
| Trigger | `on_play`, `on_cast`, `on_death` (slice); later `on_turn_start`, `on_attack`, `on_damage_dealt` | `TriggerDispatcher` enum |

...
**Design decisions:**
- **Targeting contract in commands:** `PLAY_CARD.targets` is a flat id list consumed in ability order; `splitChosenTargets` assigns `count` ids per player-targeted ability. A card whose play ability has no legal target is unplayable, creature or spell (this replaced an earlier creature-only fizzle rule; see the 2026-09-22 fix). A card never targets itself with its own play ability.
